...
namespace PartsUnlimited.Components
{
    ...
    public class AnnouncementComponent : ViewComponent
    {
        ...
        public AnnouncementComponent(IMemoryCache memoryCache, IProductsRepository productsRepository)
        {
            _cache = memoryCache;
        ...
        {
            var latestProduct = this.productsRepository.Set<Product>().ToList().OrderByDescending(a => a.Created).FirstOrDefault();
